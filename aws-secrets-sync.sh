...
UPDATED_SECRETS_STRING=$(aws secretsmanager get-secret-value --secret-id "$SSL_SECRET_MANAGER_ID" --region us-east-2 | jq -r  '.SecretString' | jq  -n ".  + $CERT_FILE_VALUES")
...
JWT_SECRET_STRING=$(echo "" | jq -n ".  + $JWT_FILE_VALUES")
